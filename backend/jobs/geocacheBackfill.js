...
const REHEAL_NONRES_KEYWORDS = [
  ...
  '경로당', '노인정', '교차로',
  // NONRES-2026-08-17b (5회차): 서울 명백 오배치 34건 개별 판정에서 남은 잔여 유형.
  //   단지명 사용례 실측 — apt_master·molit_transactions 모두 **전부 0건**(cache place_name 은
  //   출구 7·다이소 5·복지관 3·주민센터 1·치과 1·기공소 1). '입구'는 "서울대입구" 오탐 때문에
  //   과거에 제외됐지만 **'출구'는 지하철 출구 전용**이라 그 위험이 없다(강동역신동아파밀리에 실측).
  '주민센터', '복지관', '다이소', '출구', '치과', '기공소',
  // ⚠ '프라자' 는 넣지 않았다 — 단지명 실측 master 13건·molit **261건**으로 널리 쓰인다.
  // ⚠ 헬스장('…짐') 15건도 보류 — 단지명은 0건이나 '짐' 은 한 글자라 앵커(짐$)가 필요한데
  //   이 배열은 `place_name.ilike.%키워드%` 로도 쓰여 정규식 앵커를 넣을 수 없다. 근거 없이 넓히지 않는다.
];
