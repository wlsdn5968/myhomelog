...
async function kakaoGeocode({ aptName, sigungu, umdNm, address }) {
  ...
  for (const t of tries) {
    try {
      ...
      const NON_APT_PATTERNS = /빌라|사우나|어린이집|유치원|학원|마트|편의점|식당|카페|사옥|호텔|모텔|병원|약국|의원|학교|교회|성당|사찰|공원|체육관|주유소|미용실|세탁소|꽃집|충전소|주차장|정류장|정문|후문|관리사무소|경비실|놀이터/;
      const NON_APT_CATEGORY = /빌라|사우나|어린이집|유치원|학원|마트|편의점|음식점|카페|호텔|모텔|병원|약국|학교|종교|공원|체육|주유소|미용|세탁|꽃집|충전|주차|정류/;
      let chosen = null;
      let bestScore = -1;
      for (const d of docs) {
