...
> **⚠ 계획자 산술 오류 정정(2026-09-26 배포 후 실측)**: 이 문서의 B7 "오늘은 원본이 16.6개월이라 24개월 병합에 이력이 붙는 달 = 0개 → 동작 변화 0" 은 **틀렸다**. 24 − 16.6 = 7.4개월(2024-10~2025-04)이 이력에서 채워진다. 라이브 `/apt/43114-58` 거래 건수 59 → **75**(원본 59 + 이력 16, DB 실측). 이는 의도된 동작이며 "최근 24개월 요약" 라벨을 비로소 사실로 만든다. 실행자 테스트 `window-safe-readers.test.js` 의 "이력 붙을 달 0개 = 오늘의 실제 상태" 설명은 이 오류가 번진 것 — 시나리오 자체(0개일 때 동일)는 유효하므로 테스트는 유지.
